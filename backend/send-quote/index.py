...
сохраняет в quote_requests и отправляет уведомление в Telegram-бот менеджеру.
Args: event с body {kind, project_title, client_name, client_phone, client_email, comment, snapshot, total_price}
Returns: HTTP 200 с {ok: true, id} или 4xx с описанием ошибки
"""
import json
import os
import re
...
def _save_to_db(payload: Dict[str, Any], sent: bool) -> int:
    with psycopg2.connect(os.environ['DATABASE_URL']) as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "INSERT INTO quote_requests "
                "(kind, project_title, client_name, client_phone, client_email, comment, snapshot, total_price, sent_to_telegram) "
...
def handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    """Принять заявку и переслать её в Telegram."""
    ...
    qid = _save_to_db(payload, sent)

    return _resp(200, {'ok': True, 'id': qid, 'telegram_sent': sent})
